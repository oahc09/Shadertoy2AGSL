...
# AGSL uses the 'uniform' keyword for variables set via setFloatUniform().
UNIFORM_DECLARATIONS: dict[str, str] = {
    "iResolution": "uniform float3 iResolution;",
...
# Match existing uniform declarations (with or without 'uniform' keyword) to avoid duplicates.
_EXISTING_UNIFORM_RE = re.compile(
    r"^\s*(?:uniform\s+)?(?:float[234]?)\s+(iResolution|iTime|iTimeDelta|iFrame|iMouse|iDate|iSampleRate)\s*;\s*$",
    re.MULTILINE,
)
...
def inject_uniforms(source: str) -> tuple[str, list[str]]:
    ...
    # Find which are already declared (after stripping 'uniform').
    existing_decls = _EXISTING_UNIFORM_RE.findall(source)
    already_declared: set[str] = set()
    for name in existing_decls:
